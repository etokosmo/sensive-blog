...
def index(request):
    most_popular_posts = Post.objects.popular().prefetch_related(
        'author', Prefetch('tags', queryset=Tag.objects.annotate(
            post_count=Count('posts'))))[:5].fetch_with_comments_count()

    most_fresh_posts = Post.objects.annotate(
        comments_count=Count('comments')).order_by(
        '-published_at').prefetch_related(
        'author', Prefetch('tags', queryset=Tag.objects.annotate(
            post_count=Count('posts'))))[:5]

    most_popular_tags = Tag.objects.popular()[:5][::-1]
...
def post_detail(request, slug):
    ...
    related_tags = post.tags.all().annotate(post_count=Count('posts'))
    ...
    most_popular_tags = Tag.objects.popular()[::-1]
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().prefetch_related(
        'author', Prefetch('tags', queryset=Tag.objects.annotate(
            post_count=Count('posts'))))[:20].fetch_with_comments_count()
